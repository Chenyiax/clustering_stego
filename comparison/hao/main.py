...
torch.save(stego_module.gcn_net, f"../../model/gcn_{args.model}_{args.dataset}.pth")
...
for epoch in range(num_epochs):
    ...
    for data, target in tqdm(train_loader):
        ...
        secret_bits = stego_module.generate_watermark().to(device)

        # 前向传播
        output = model(data)

        extract_secret_bits = stego_module.extract_watermark(stegolayer)

        # 水印提取误差
        loss_data_mse = torch.mean((secret_bits - extract_secret_bits) ** 2)
        ...
        # 总损失
        loss = loss_classification + args.beta * loss_data_mse
    ...
    predictions = (extract_secret_bits > 0.5).float()
    correct = (secret_bits == predictions).sum().item()
...
torch.save(model.state_dict(), f"model/{args.model}_{args.dataset}_with_secret_hao.pth")
torch.save({"secret_bits": stego_module.secret_bits, "matrix": stego_module.adjacency_matrix},
           f'data/secret_{args.model}_{args.dataset}_hao.pth')
torch.save(stego_module.gcn_net, f"model/gcn_{args.model}_{args.dataset}.pth")
torch.save(loss_list, f"data/loss_{args.model}_{args.dataset}_with_secret_hao.pth")