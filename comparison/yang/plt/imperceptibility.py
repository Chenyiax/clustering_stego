...
parser.add_argument('--model', default="vgg16", type=str, help='模型, 可选:alexnet, vgg16, resnet18')
...
model1.load_state_dict(torch.load(f"../../../model/{args.model}_cifar10_with_secret_{args.beta}.pth"))
model2.load_state_dict(torch.load(f"../../../model/{args.model}_cifar10_without_secret.pth"))

if args.model == 'alexnet':
    watermarklayer = model1.features[3]
    originlayer = model2.features[3]
elif args.model == 'vgg16':
    watermarklayer = model1.features[12]
    originlayer = model2.features[12]
elif args.model == 'resnet18':
    watermarklayer = model1.layer2[0].conv2
    originlayer = model2.layer2[0].conv2
else:
